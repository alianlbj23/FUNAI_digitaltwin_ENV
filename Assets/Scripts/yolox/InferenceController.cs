...
public class InferenceController : MonoBehaviour
{
    ...
    [Header("Data to ROS")]
    public YoloToROS yoloInformation;
    ...
    private void Update()
    {
        ...
        if (!AreComponentsValid() || !cameraTexture) return;

        // Get the input image and dimensions
        var imageTexture = cameraTexture;
        ...
        Vector2 screenDims = new Vector2(cameraTexture.width, cameraTexture.height);

        // 先註解掉，避免頻道被占用
        // yoloInformation.ShowYoloInformation(bboxInfoArray);
    ...
    private bool AreComponentsValid()
    {
        ...
        {
            Debug.LogError("InferenceController requires ImageProcessor and ModelRunner components.");
            return false;
        }
        return true;
    }
    ...
    private void ProcessInputImage(RenderTexture inputTexture, Texture imageTexture, Vector2Int sourceDims, Vector2Int inputDims)
    {
        ...
        RenderTexture sourceTexture = PrepareInputTexture(sourceDims);
    ...
    public void UpdateCameraTexture(RenderTexture renderTexture)
    {
